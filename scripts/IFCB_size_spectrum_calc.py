...
def log_bins_func(start, bin_number):
   ...
   for i in iter:
       y = limits_list[i]
       x = pow(10,(np.log10(y) + np.log10(pow(2,0.25))))
...
# create a list of IFCBs projects that we have access to, based on project_list_all.xlsx
...
EcopartSizeBins_mm = np.array([0.001000, 0.001260, 0.001590, 0.002000, 0.002520, 0.003170, 0.004000, 0.005040, 0.006350, 0.008000
...
#create depth bins for the sampling
depths = [0, 25, 50, 100, 200, 500, 1000, 3000, 8000]
...
for id in id_list:
    ...
    data_clean['biovol_um3'] = (4/3)* 3.14 * ((data_clean['ESD_um']/2)**2) # based on sphere volume, likely better ways
    #to do this
    ...
    PD_SizeBins_um = log_bins_func((min(data_clean['ESD_um'])), 50)# number of bins work for this data
    SizeBins_um = np.array(PD_SizeBins_um)
    # for methods (2) and (3) restrict the size classes based on the max and minimum size of the particles in the project
    #SizeBins_um = SizeBins_um[(SizeBins_um >=  min(data_clean['ESD_um'])-0.001) & \
                                             #(SizeBins_um <=  max(data_clean['ESD_um'])+0.001)]
    # create a categorical variable, which are bins defined by the numbers on the sizeClasses list
    # Assign bin to each data point based on object_area value
    data_clean['sizeClasses'] = pd.cut(x=data_clean[('ESD_um')],
