...
def matchup_profileid(id,profiles):
    match=get_close_matches(id,profiles)
    if len(match)>0:
        test=[matchup for matchup in match if any(pd.Series(matchup).str.contains(id)) | any(pd.Series(id).str.contains(matchup))]
        if len(test):
            matchup=test[0]
        else:
            matchup = ''
...
def consolidate_ecotaxa_project(project_id,standardizer=df_standardizer_ecotaxa,sheetname='ecotaxa',run_matchup=False,run_macro=False,upload_metadata=False,localpath_metadata=Path(cfg['raw_dir']).expanduser()/cfg['UVP_consolidation_subdir']):
   ...
   if project_id in standardizer.index and str(standardizer.at[project_id, "External_project"])!='nan':
       ...
       if (path_to_external_project.name=='ecopart') and len(path_external_export) and any(pd.Series(str(standardizer.at[project_id, "External_project"]).split(';')).isin(list(df_standardizer_ecopart.index.astype(str)))):
           ...
           # Some profile id do not match with the metadata id (example project 8800(Ecotaxa)/605(Ecopart))
           if any(df_ecopart.profileid.isin(depthprofile.reset_index().profileid)==False):
               profileid_matchup=dict(map(lambda id: (id,matchup_profileid(id,profiles=depthprofile.reset_index().profileid)) if len(matchup_profileid(id,profiles=depthprofile.reset_index().profileid))>0 else ('','') ,df_ecopart.profileid.unique()))
               df_ecopart['profileid']=df_ecopart.profileid.map(profileid_matchup)

           df_ecopart = pd.merge(df_ecopart, depthprofile.reset_index(), how='left', left_on=['Project_ID','profileid'], right_on=['pprojid','profileid'])
           ...
           if Path(path_to_export).is_file() and len(df_ecopart) and len(df_metadata):
              ...
              df_ecotaxa['sample_profileid']=np.where(df_ecotaxa.sample_profileid.astype(str).str.startswith('hdr'),df_ecotaxa.sample_profileid.astype(str),'hdr'+df_ecotaxa.sample_profileid.astype(str))
              ...
              df_ecopart['Profile'] ='hdr'+df_ecopart.filename.astype(str)
              df_ecotaxa=df_ecotaxa[df_ecotaxa.sample_profileid.astype(str).isin(df_ecopart.Profile.astype(str).unique())].reset_index(drop=True)
              df_ecopart = df_ecopart[df_ecopart.Profile.astype(str).isin( df_ecotaxa.sample_profileid.astype(str).unique())].reset_index(drop=True)
              ...
              if all(pd.Series(['object_bru_id','object_bru_area','object_image_name','object_image_index','object_corrected_depth','object_corrected_min_depth_bin','object_corrected_max_depth_bin','object_volume_bin','object_imgcount_bin']).isin(list(df_ecotaxa.columns))):
                  ...
                  integration_time=df_ecopart_extended[['Profile','sampledate']].drop_duplicates().groupby(['Profile']).apply(lambda x:pd.Series({'integrationtime':int(pd.to_datetime(x.sampledate,format="%Y-%m-%d %H:%M:%S").diff().dt.seconds.min()) if len(x)>1 else 1}))
                  if len(integration_time): # Cannot round at frequency 0
                      integration_time.loc[integration_time.integrationtime==0,'integrationtime']=1
                  df_ecotaxa=pd.merge(df_ecotaxa,integration_time.reset_index(),how='left',on='Profile')
                  sampledate_dict=dict(map(lambda row: (tuple([row[1].object_date,row[1].object_time,row[1].integrationtime]),pd.to_datetime(row[1].object_date.astype(str).zfill(6)+row[1].object_time.astype(str).zfill(6),format='%Y%m%d%H%M%S').strftime('%Y-%m-%d %H:%M:%S')),df_ecotaxa[['object_date', 'object_time', 'integrationtime']].drop_duplicates().iterrows()))#dict(map(lambda row: (tuple([row[1].object_date,row[1].object_time,row[1].integrationtime]),pd.to_datetime(row[1].object_date.astype(str).zfill(6)+row[1].object_time.astype(str).zfill(6),format='%Y%m%d%H%M%S').floor("{}S".format((row[1].integrationtime.astype(str)))).strftime('%Y-%m-%d %H:%M:%S')),df_ecotaxa[['object_date', 'object_time', 'integrationtime']].drop_duplicates().iterrows()))
                  df_ecotaxa['sampledate']=df_ecotaxa[['object_date', 'object_time', 'integrationtime']].apply(tuple, axis=1).map(sampledate_dict)

                  df_ecotaxa['sampledate'] =np.where(df_ecotaxa.organizedbydepth == False,df_ecotaxa['sampledate'], pd.merge(df_ecotaxa[[ 'sample_id']],df_ecopart_extended[['profileid','sampledate']].groupby(['profileid']).first(),how='left',left_on='sample_id',right_on='profileid')['sampledate'])
                  ...
                  df_depth_bins=pd.concat([df_ecopart_extended[df_ecopart_extended.organizedbydepth==True][['Profile','sampledate','object_corrected_min_depth_bin']].groupby(['Profile','sampledate']).first().reset_index(),df_ecopart_extended[df_ecopart_extended.organizedbydepth==False][['Profile','sampledate','object_corrected_min_depth_bin']].drop_duplicates()],ignore_index=True,axis=0) if any(df_ecotaxa.organizedbydepth==False) else df_ecopart_extended[['Profile','sampledate','object_corrected_min_depth_bin']].groupby(['Profile','sampledate']).first()#pd.concat([df_ecopart_extended[df_ecopart_extended.organizedbydepth==True][['profileid','sampledate','object_corrected_min_depth_bin']].groupby(['profileid','sampledate']).first().reset_index(),df_ecopart_extended[df_ecopart_extended.organizedbydepth==False][['profileid','sampledate','object_corrected_min_depth_bin']].drop_duplicates()],ignore_index=True,axis=0) if any(df_ecotaxa.organizedbydepth==False) else df_ecopart_extended[['profileid','sampledate','object_corrected_min_depth_bin']].groupby(['profileid','sampledate']).first()
                  df_ecotaxa['object_corrected_min_depth_bin'] =np.where(df_ecotaxa.organizedbydepth==False,pd.merge(df_ecotaxa[['Profile','sampledate']],df_depth_bins,how='left',right_on=['Profile','sampledate'],left_on=['Profile','sampledate'])['object_corrected_min_depth_bin'],pd.cut(df_ecotaxa['object_corrected_depth'], bins=list(np.arange(0, 6501, step=1)),right=False,labels=np.arange(0, 6500, step=1)).astype(str).astype(int))
                  df_ecotaxa['object_corrected_max_depth_bin'] =np.where(df_ecotaxa.organizedbydepth==False,pd.merge(df_ecotaxa[['Profile','sampledate']],df_depth_bins,how='left',right_on=['Profile','sampledate'],left_on=['Profile','sampledate'])['object_corrected_min_depth_bin'],df_ecotaxa['object_corrected_min_depth_bin']+1)
                  df_ecopart_extended = df_ecopart[(df_ecopart.area <= sm_zoo) | (df_ecopart[ ['Profile','sampledate', 'object_corrected_min_depth_bin', 'object_corrected_max_depth_bin']].apply(tuple, axis=1).isin(df_ecotaxa[ ['Profile','sampledate', 'object_corrected_min_depth_bin','object_corrected_max_depth_bin']].apply( tuple, axis=1)) == False)].rename( columns={'area': 'object_bru_area', 'nbr': 'object_number'}).reset_index(drop=True)
              ...
              df_ecopart_extended =pd.merge(df_ecopart_extended.drop(columns=[fields_from_ecotaxa['Pixel']]).assign(sample_id=df_ecopart_extended.profileid,object_annotation_category='',object_annotation_status='unclassified').astype({'Profile':str,'sample_id':str}),df_ecotaxa.astype({'Profile':str,'sample_id':str,'sample_stationid':str})[[fields_from_ecotaxa['Pixel'],'Profile']+[column for column in df_ecotaxa.columns if (len(re.findall( r'sample_|acq_|process_', column))>0) and (column not in df_ecopart_extended.columns)]].drop_duplicates(),how='left',left_on=['Profile','sample_id'],right_on=['Profile','sample_id'])
              df_ecopart_extended = pd.concat([df_ecopart_extended.drop(columns=['filename']), df_ecotaxa.astype({'sample_profileid':str,'sample_id':str,'sample_stationid':str}).drop(columns=['filename']).dropna(subset=[ 'object_volume_bin', 'object_imgcount_bin'])], axis=0,ignore_index=True)#pd.concat([pd.merge(df_ecopart_extended.assign(sample_id=df_ecopart_extended.profileid,object_annotation_category='',object_annotation_status='unclassified'),df_ecotaxa[['object_lat','object_lon','object_date','object_time']+[column for column in df_ecotaxa.columns if len(re.findall( r'sample_|acq_|process_', column))>0]].drop_duplicates(),how='left',on='sample_id'), df_ecotaxa], axis=0)
