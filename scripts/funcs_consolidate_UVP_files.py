   ...
   if project_id in standardizer.index and str(standardizer.at[project_id, "External_project"])!='nan':
       ...
       if (path_to_external_project.name=='ecopart') and len(path_external_export) and all(pd.Series(str(standardizer.at[project_id, "External_project"]).split(';')).isin(list(df_standardizer_ecopart.index.astype(str)))):
           ...
           if Path(path_to_export).is_file() and len(df_ecopart) and len(df_metadata):
              ...
              # Attention: For mixture of organizedbydepth, False value profiles are dropped in Ecopart
              df_ecotaxa=df_ecotaxa[df_ecotaxa[fields_from_ecotaxa['Sample']].astype(str).isin(list(df_ecopart[['profileid']].drop_duplicates().profileid.astype(str).values))]
              ...
              df_ecopart_extended = pd.concat([pd.merge(df_ecopart_extended.drop(columns=[fields_from_ecotaxa['Pixel']]).assign(sample_id=df_ecopart_extended.profileid,object_annotation_category='',object_annotation_status='unclassified'),df_ecotaxa[[fields_from_ecotaxa['Pixel']]+[column for column in df_ecotaxa.columns if (len(re.findall( r'sample_|acq_|process_', column))>0) and (column not in df_ecopart_extended.columns)]].drop_duplicates(),how='left',on='sample_id'), df_ecotaxa.dropna(subset=[ 'object_volume_bin', 'object_imgcount_bin'])], axis=0)#pd.concat([pd.merge(df_ecopart_extended.assign(sample_id=df_ecopart_extended.profileid,object_annotation_category='',object_annotation_status='unclassified'),df_ecotaxa[['object_lat','object_lon','object_date','object_time']+[column for column in df_ecotaxa.columns if len(re.findall( r'sample_|acq_|process_', column))>0]].drop_duplicates(),how='left',on='sample_id'), df_ecotaxa], axis=0)
              ...
              df_ecopart_extended =df_ecopart_extended[['filename']+list(df_ecotaxa.columns)].sort_values(['object_time', 'object_corrected_min_depth_bin','object_bru_area'],ascending=[True,True,False]).reset_index(drop=True)
