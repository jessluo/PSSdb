...
for instrument in natsorted(os.listdir(Path(cfg['raw_dir']).expanduser() / cfg['gridded_subdir'])):
    ...
    if len(NBSS_binned_all):
        ...
        df_metadata = pd.DataFrame({'Variables': ['Taxon or PFT','Validation_percentage','year','month','ocean','latitude','longitude','min_depth','max_depth','n','biovolume_size_class','normalized_biovolume_mean','normalized_biovolume_std','biomass_mid','range_biomass_bin','normalized_biomass_mean','normalized_biomass_std','equivalent_circular_diameter_mean','normalized_abundance_mean','normalized_abundance_std','NBSS_slope_mean','NBSS_slope_std','NBSS_intercept_mean','NBSS_intercept_std','NBSS_r2_mean','NBSS_r2_std','PSD_slope_mean','PSD_slope_std','PSD_intercept_mean','PSD_intercept_std','PSD_r2_mean','PSD_r2_std'],
             'Units/Values/Timezone': ['','[0-1]','yyyy','mm','','decimal degree','decimal degree','meter','meter','','cubic micrometer','cubic micrometer per liter per cubic micrometer','cubic micrometer per liter per cubic micrometer','gram','gram','gram per liter per gram','gram per liter per gram','micrometer','particles per liter per micrometer','particles per liter per micrometer','liter per cubic micrometer (biovolume) or liter per gram (biomass)','liter per cubic micrometer (biovolume) or liter per gram (biomass)','log_10 of per liter','log_10 of per liter','','','log_10 of per liter per square micrometer','log_10 of per liter per square micrometer','log_10 of particles per liter per micrometer','log_10 of particles per liter per micrometer','',''],
             'Description': ['Taxonomic affiliation according to the World Register of Marine Species database (for Taxon) or to functional groups (for PFT)','Percentage of image classification validation','Year of the temporal bin','Month of the temporal bin','Study area according to the Global Oceans and Seas of the standard geo-referenced marine regions website (https://www.marineregions.org/)','latitude','longitude','Minimum depth','Maximum depth','Number of individual sub-bins (weekly, half degree) grouped to compute the average (std) size spectrum and parameters','Midpoint of the biovolume size bin','Average of the Normalized biovolume estimates in final bins (month year, 1x1 degree)','Standard deviation of the Normalized biovolume estimates in final bins (month year, 1x1 degree)','Midpoint of the biomass bin','Biomass bin width (can be used to calculate total biomass by multiplying with normalized biomass)','Average of the Normalized biomass estimates in final bins (month year, 1x1 degree)','Standard deviation of the Normalized biomass estimates in final bins (month year, 1x1 degree)','Midpoint of the equivalent circular diameter size bin','Average of the Normalized abundance estimates in final bins (month year, 1x1 degree)','Standard deviation of the Normalized abundance estimates in final bins (month year, 1x1 degree)','Average of the Normalized biovolume/biomass size spectrum slopes in final bins (month year, 1x1 degree)','Standard deviation of the Normalized biovolume/biomass size spectrum slopes in final bins (month year, 1x1 degree)','Average of the Normalized biovolume/biomass size spectrum intercepts in final bins (month year, 1x1 degree)','Standard deviation of the Normalized biovolume/biomass size spectrum intercepts in final bins (month year, 1x1 degree)','Average of the linear fit determination coefficients in final bins (month year, 1x1 degree)','Standard deviation of the linear fit determination coefficients in final bins (month year, 1x1 degree)','Average of the particle size distribution slopes in final bins (month year, 1x1 degree)','Standard deviation of the particle size distribution slopes in final bins (month year, 1x1 degree)','Average of the particle size distribution intercepts in final bins (month year, 1x1 degree)','Standard deviation of the particle size distribution intercepts in final bins (month year, 1x1 degree)','Average of the linear fit determination coefficients in final bins (month year, 1x1 degree)','Standard deviation of the linear fit determination coefficients in final bins (month year, 1x1 degree)']})
        # Create readme file:
