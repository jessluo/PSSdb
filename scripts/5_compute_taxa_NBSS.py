...
for instrument in os.listdir(Path(cfg['raw_dir']).expanduser() / cfg['gridded_subdir']):
    ...
    for i in tqdm(grid_list):
        ...
        df_bins[['biomass_mid', 'range_biomass_bin']] = data_bins[['biomass_mid', 'range_biomass_bin']]
    ...
    NBSS_raw_biomass = NBSS_binned_all_biomass.filter(['date_bin', 'midLatBin', 'midLonBin','Taxon','PFT','Validation_percentage', 'size_class_mid', 'ECD_mid','biomass_mid', 'NB', 'PSD','Min_obs_depth', 'Max_obs_depth'], axis=1)
    ...
    if len(NBSS_raw_PFT):
        ...
        lin_fit_1b_PFT= lin_fit_data_PFT.groupby(['PFT','Validation_percentage']).apply(lambda x: stats_linfit_func(x, bin_loc=bin_loc, group_by=group_by)).reset_index().drop(columns=['level_2'])
    ...
    if len(NBSS_raw_biomass):
        ...
        lin_fit_1b_class_biomass= lin_fit_data_biomass.groupby(['Taxon','Validation_percentage','Total_biomass']).apply(lambda x: stats_linfit_func(x, bin_loc=bin_loc, group_by=group_by)).reset_index().drop(columns=['level_3'])
    ...
    if len(NBSS_raw_biomass_PFT):
        ...
        lin_fit_1b_biomass_PFT= lin_fit_data_biomass_PFT.groupby(['PFT','Validation_percentage','Total_biomass']).apply(lambda x: stats_linfit_func(x, bin_loc=bin_loc, group_by=group_by)).reset_index().drop(columns=['level_3'])
