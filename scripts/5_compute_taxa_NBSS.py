...
for instrument in natsorted(os.listdir(Path(cfg['raw_dir']).expanduser() / cfg['gridded_subdir'])):
    ...
    if len(NBSS_binned_all):
        ...
        df_metadata = pd.DataFrame({'Variables': ['Taxon or PFT','Validation_percentage','year','month','ocean','latitude','longitude','min_depth','max_depth','n','biovolume_size_class','normalized_biovolume_mean','normalized_biovolume_std','biomass_mid','range_biomass_bin','normalized_biomass_mean','normalized_biomass_std','equivalent_circular_diameter_mean','normalized_abundance_mean','normalized_abundance_std','NBSS_slope_mean','NBSS_slope_std','NBSS_intercept_mean','NBSS_intercept_std','NBSS_r2_mean','NBSS_r2_std','PSD_slope_mean','PSD_slope_std','PSD_intercept_mean','PSD_intercept_std','PSD_r2_mean','PSD_r2_std'],
             'Units/Values/Timezone': ['','[0-1]','yyyy','mm','','decimal degree','decimal degree','meter','meter','','cubic micrometer','cubic micrometer per liter per cubic micrometer','cubic micrometer per liter per cubic micrometer','gram','gram','gram per liter per gram','gram per liter per gram','micrometer','particles per liter per micrometer','particles per liter per micrometer','liter per cubic micrometer (biovolume) or liter per gram (biomass)','liter per cubic micrometer (biovolume) or liter per gram (biomass)','per liter','per liter','','','per liter per micrometer','per liter per micrometer','particles per liter per micrometer','particles per liter per micrometer','',''],
             'Description': ['Taxonomic affiliation according to the World Register of Marine Species database (for Taxon) or to functional groups (for PFT)','Percentage of image classification validation','Year of the temporal bin','Month of the temporal bin','Study area according to the Global Oceans and Seas of the standard georeferenced marine regions website (https://www.marineregions.org/)','latitude','longitude','Minimum depth','Maximum depth','Number of individual sub-bins (weekly, half degrees) grouped to compute the average (std) size spectrum and parameters','Midpoint of the biovolume size bin','Average of the Normalized biovolume estimates in final bins (month year, 1 degrees)','Standard deviation of the Normalized biovolume estimates in final bins (month year, 1 degrees)','Midpoint of the biomass bin','Biomass bin width (can be used to calculate total biomass by multiplying with normalized biomass)','Average of the Normalized biomass estimates in final bins (month year, 1 degrees)','Standard deviation of the Normalized biomass estimates in final bins (month year, 1 degrees)','Midpoint of the equivalent circular diameter size bin','Average of the Normalized abundance estimates in final bins (month year, 1 degrees)','Standard deviation of the Normalized abundance estimates in final bins (month year, 1 degrees)','Average of the Normalized biovolume/biomass size spectrum slopes in final bins (month year, 1 degrees)','Standard deviation of the Normalized biovolume/biomass size spectrum slopes in final bins (month year, 1 degrees)','Average of the Normalized biovolume/biomass size spectrum intercepts in final bins (month year, 1 degrees)','Standard deviation of the Normalized biovolume/biomass size spectrum intercepts in final bins (month year, 1 degrees)','Average of the linear fit determination coefficients in final bins (month year, 1 degrees)','Standard deviation of the linear fit determination coefficients in final bins (month year, 1 degrees)','Average of the particle size distribution slopes in final bins (month year, 1 degrees)','Standard deviation of the particle size distribution slopes in final bins (month year, 1 degrees)','Average of the particle size distribution intercepts in final bins (month year, 1 degrees)','Standard deviation of the particle size distribution intercepts in final bins (month year, 1 degrees)','Average of the linear fit determination coefficients in final bins (month year, 1 degrees)','Standard deviation of the linear fit determination coefficients in final bins (month year, 1 degrees)']})
        # Create readme file:
        if not Path(NBSSpath.parent / 'README.txt').is_file():
            with open(Path(NBSSpath.parent / str('README.txt')), 'w') as file:
                file.write( "README file for project taxa-specific NBSS estimates (First created on December 2023):\n\nThis directory contains the taxa-specific products (1a: size or biomass spectrum and 1b: derived linear fit parameters) of gridded datasets.\nEach table include the following variables:\n\n{}\n\nContact us: contact@pssdb.net".format('\n'.join(list( df_metadata[['Variables', 'Units/Values/Timezone', 'Description']].apply(lambda x: str(x.Variables) + " (" + str( x['Units/Values/Timezone']).strip() + "): " + x.Description if len(x['Units/Values/Timezone']) else str(x.Variables) + ": " + x.Description,axis=1).values))))
    ...
    if len(NBSS_binned_all_biomass):
        ...
        NBSS_binned_all_biomass=NBSS_binned_all_biomass_thres.groupby(['date_bin', 'Station_location', 'midLatBin', 'midLonBin', 'Min_obs_depth', 'Max_obs_depth', 'Taxon', 'Validation_percentage']).apply(lambda x: threshold_func(x)).reset_index(drop=True).sort_values( ['date_bin', 'Station_location','midLatBin', 'midLonBin', 'Min_obs_depth', 'Max_obs_depth', 'Taxon','biomass_mid']).reset_index(drop=True)
        ...
        NBSS_binned_all_PFT_biomass=NBSS_binned_all_thres_PFT_biomass.groupby(['date_bin', 'Station_location', 'midLatBin', 'midLonBin', 'Min_obs_depth', 'Max_obs_depth', 'PFT', 'Validation_percentage']).apply(lambda x: threshold_func(x)).reset_index(drop=True).sort_values( ['date_bin', 'Station_location','midLatBin', 'midLonBin', 'Min_obs_depth', 'Max_obs_depth', 'PFT','biomass_mid']).reset_index(drop=True)
    ...
    if len(NBSS_raw_weight):
        ...
        NBSS_1a_class_weight.to_csv(NBSSpath /'Class' / str(instrument + '_1a_Weight-distribution_v' + currentYear + '-' + currentMonth + '.csv'),index=False)
        lin_fit_1b_class_weight.to_csv(NBSSpath / 'Class' / str(instrument + '_1b_Weight-spectra-fit_v' + currentYear + '-' + currentMonth + '.csv'),index=False)
    ...
    if len(NBSS_raw_weight_PFT):
        ...
        # Save data levels
        NBSS_1a_weight_PFT.to_csv(NBSSpath /'PFT' / str(instrument + '_1a_Weight-distribution_v' + currentYear + '-' + currentMonth + '.csv'),index=False)
        lin_fit_1b_weight_PFT.to_csv(NBSSpath / 'PFT' / str(instrument + '_1b_Weight-spectra-fit_v' + currentYear + '-' + currentMonth + '.csv'),index=False)
