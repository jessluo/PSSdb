...
nbss_all = df_subset.groupby(group + list(df_bins.columns) + ['Group_index', 'Sampling_lower_size', 'Sampling_upper_size'],dropna=False).apply(lambda x: pd.Series({'size_class_pixel': (x.Pixel.unique()[0]) * 1e-03 * x.size_class_mid.unique()[0],
                                                                                                                                                                    'NBSS_count': x.ROI_number.sum(),
                                                                                                                                                                    'sum_biovolume': sum(x.Biovolume * x.ROI_number),
                                                                                                                                                                    'volume': x.cumulative_volume.unique()[0],
                                                                                                                                                                    'NBSS': sum(x.Biovolume * x.ROI_number) / x.cumulative_volume.unique()[0] / (x.range_size_bin.unique())[0],
                                                                                                                                                                    'NBSS_std': np.std(list(map(lambda count: np.random.normal(loc=np.random.choice(np.repeat(x.Biovolume, x.ROI_number), size=count, replace=True), scale=(1 / 3) * np.pi * (np.sqrt((x.Pixel.unique()[0]) * 1e-03 * x.size_class_mid.unique()[0]) ** 3) / ( (1e-03 * x.Pixel.unique()[0]) ** 3), size=None).sum() / x.cumulative_volume.unique()[0] / (x.range_size_bin.unique())[0], np.random.poisson(lam=sum(x.ROI_number), size=niter))))})).reset_index()
...
nbss_int = nbss_sum.groupby(['Instrument','Group_index' ,'Sampling_datetime','Date_subbin','Date_bin','Longitude','Longitude_subbin','Longitude_bin','Latitude','Latitude_subbin','Latitude_bin'] + list( df_bins.columns)).apply(lambda x: pd.Series({'volume': x.volume.unique()[0],
                                                                                                                                                                                                                                                       'size_class_pixel': np.nanmean(x.size_class_pixel),
                                                                                                                                                                                                                                                       'NBSS_count': sum(x.NBSS_count),
...
nbss_avg=nbss_int.groupby(['Instrument','Date_subbin','Date_bin','Longitude_subbin','Longitude_bin','Latitude_subbin','Latitude_bin','Depth_range_min','Depth_range_max','size_class_pixel'] + list( df_bins.columns)).apply(lambda x:pd.Series({'NBSS_count': np.nanmean(x.NBSS_count),
                                                                                                                                                                                                                                                 'NBSS': np.nanmean(x.NBSS_sum)})).reset_index()
nbss_avg=nbss_avg.groupby(['Instrument','Date_bin','Longitude_bin','Latitude_bin','Depth_range_min','Depth_range_max','size_class_pixel'] + list( df_bins.columns)).apply(lambda x:pd.Series({'NBSS_count': np.nanmean(x.NBSS_count),
                                                                                                                                                                                              'NBSS': np.nanmean(x.NBSS)})).reset_index()
